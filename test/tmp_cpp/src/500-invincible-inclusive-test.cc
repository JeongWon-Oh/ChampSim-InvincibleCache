...
SCENARIO("Invincible Cache Test on Inclusive LLC") {
  GIVEN("Three cache levels with LLC filled by cpu 0") {
    ...
    WHEN("cpu1 trys to access LLC") {
      ...
      test.cpu = 1;
      test.instr_id = id++;
      test.clusivity = champsim::inclusivity::inclusive;
      ...
      THEN("cpu1 is blocked") {
        REQUIRE(!cpu1_l1d.block.front().valid);
        REQUIRE(!cpu1_l2.block.front().valid);
      }
      ...
      AND_WHEN("cpu0 trys to access LLC") {
        ...
        test2.cpu = 0;
        test2.instr_id = id++;
        test2.clusivity = champsim::inclusivity::inclusive;
        ...
        AND_WHEN("after random eviction frequency") {
          for (uint64_t i = 0; i < RANDOM_EVICTION_FREQ; ++i)
            for (auto elem : elements)
              elem->_operate();
